...
SplatLayerFactory::install(MapNode* mapNode)
{
    ...
    if (splattingOK)
    {
        ...
        bool coverageOK = mapNode->getTerrainEngine()->getResources()->reserveTextureImageUnit(_splatTexUnit, "Splat Coverage Data");

        bool lutOK = mapNode->getTerrainEngine()->getResources()->reserveTextureImageUnit(_lutTexUnit, "Splat LUT");
        ...
        if (coverageOK && lutOK)
        {
            ...
            splatting.define("OE_USE_NORMAL_MAP", mapNode->getTerrainEngine()->normalTexturesRequired());
...
SplatLayerFactory::uninstall(MapNode* mapNode)
{
    if ( mapNode )
    {
        ...
        if ( _splatTexUnit >= 0 )
        {
            mapNode->getTerrainEngine()->getResources()->releaseTextureImageUnit( _splatTexUnit );
            _splatTexUnit = -1;
        }
    }
}

...
CroppingLocator::CroppingLocator( osgTerrain::Locator* locator, const GeoExtent& dataExtent, const GeoExtent& croppedExtent ) :
osgTerrain::Locator( *locator ),
_dataExtent( dataExtent ),
_croppedExtent( croppedExtent )
{
    // assume they are the same SRS
    _x0 = (_croppedExtent.xMin()-_dataExtent.xMin())/_dataExtent.width();
    _x1 = (_croppedExtent.xMax()-_dataExtent.xMin())/_dataExtent.width();
    _y0 = (_croppedExtent.yMin()-_dataExtent.yMin())/_dataExtent.height();
    _y1 = (_croppedExtent.yMax()-_dataExtent.yMin())/_dataExtent.height();
...
CroppingLocator::convertModelToLocal(const osg::Vec3d& world, osg::Vec3d& local) const
{
    ...
    //_locator->_inverse.invert( _transform );

    bool ok = osgTerrain::Locator::convertModelToLocal( world, local );

    //osg::notify(osg::NOTICE) << "x = " << local.x() << ", y = " << local.y()  << " ... ";

    local.x() = (local.x() - _transform.ptr()[12]) / _transform.ptr()[0];
    local.y() = (local.y() - _transform.ptr()[13]) / _transform.ptr()[5];

    local.x() = _x0 + local.x()*(_x1 - _x0);
    local.y() = _y0 + local.y()*(_y1 - _y0);

...
const char * LoaderFile::load( const char *name ) {
    ...
    std::string fullPath = osgEarth::Support::getFullPath(_referrer, std::string(name));
...
void Template::load_recursive( const char *name, vector<Tokenizer*> & files, vector<Node*> & nodes ) {
    ...
    std::string referrer = osgEarth::Support::getFullPath(loader.getReferrer(), std::string(name));

...
#if 0
void
GeometryPool::getPooledPatch(const TileKey&               tileKey,
                             unsigned                     size,
                             const MapInfo&               mapInfo,
                             osg::ref_ptr<osg::Geometry>& out)
{
    // convert to a unique-geometry key:
    GeometryKey geomKey;
    createKeyForTileKey( tileKey, size, true, mapInfo, geomKey );
    ...
    {
        // Not found. Create it.
        out = createPatch( tileKey, size, mapInfo, 0L );
        _geometryMap[ geomKey ] = out.get();
...
#if 0
osg::Geometry*
GeometryPool::createPatch(const TileKey& tileKey,
                          unsigned       size,
                          const MapInfo& mapInfo,
                          MaskGenerator* maskSet) const
{
    ...
    geom->addPrimitiveSet( new osg::DrawArrays(mode, 0, numVerts) );
    ...
    // vert is masked: 0=yes, 1=no
    bool populateTexCoords = false;
    if ( !_sharedPatchCoords.valid() )
    {
        _sharedPatchCoords = new osg::Vec3Array();
        _sharedPatchCoords->reserve( numVerts );
        populateTexCoords = true;
    }    
    osg::Vec3Array* texCoords = _sharedPatchCoords.get();

    geom->setTexCoordArray( 0, texCoords );

    //float shift = 0.5f / (float)(_tileSize-1);
    
    for(unsigned row=0; row<_tileSize; ++row)
    {
        float ny = (float)row/(float)(size-1);

        for(unsigned col=0; col<_tileSize; ++col)
        {
            float nx = (float)col/(float)(size-1);

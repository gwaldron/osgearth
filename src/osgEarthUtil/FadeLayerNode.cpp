...
void FadeLayerNode::removeLayer(unsigned int layer)
{
    if (layer < 0 || layer >= _enabled.size()) return;

    _enabled.erase(_enabled.begin() + layer);
    _opacity.erase(_opacity.begin() + layer);
    updateStateSet();
}

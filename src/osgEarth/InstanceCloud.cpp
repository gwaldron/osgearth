...
InstanceCloud::GenBuffer::clearTile(unsigned tileNum)
{
    ...
    _buffer->bind();
    _glClearBufferSubData(GL_SHADER_STORAGE_BUFFER, GL_R8I, offset, numBytesPerTile, GL_RED, GL_UNSIGNED_BYTE, &data);
}
...
InstanceCloud::RenderBuffer::allocate(unsigned numInstances, GLsizei alignment, osg::State& state)
{
    ...
    if (_requiredSize > _allocatedSize)
    {
        ...
        _buffer->bind();
        _glBufferStorage(GL_SHADER_STORAGE_BUFFER, _allocatedSize, NULL, 0);
    }
}
...
InstanceCloud::InstanceCloud()
{
    osg::setGLExtensionFuncPtr(
        ...
        "glDispatchComputeIndirect", "glDispatchComputeIndirectARB");
}

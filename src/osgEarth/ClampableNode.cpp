...
{
    /**
     * When draping is enabled, the actual graph goes under an OverlayProxy
     * group. It tracks the accumulated stateset and nodemask of the ClampableNOde
...
ClampableNode::ClampableNode( MapNode* mapNode, bool clamped ) :
_newClamped( clamped ),
_clamped   ( false ),
_dirty     ( false )
{
    ...
    // to "track" the traversal state of the ClampableNode itself.
...
void
ClampableNode::setMapNode( MapNode* mapNode )
{
    MapNode* oldMapNode = getMapNode();

    if ( oldMapNode != mapNode )
    {
        if ( oldMapNode && _clamped && _overlayProxyContainer->getNumParents() > 0 )
        {
            osg::Group* group = oldMapNode->getOverlayDecorator()->getGroup<ClampingTechnique>();
            if ( group )
...
void
ClampableNode::applyChanges()
{
    _clamped = _newClamped;

    if ( getMapNode() )
    {
        if ( _clamped && _overlayProxyContainer->getNumParents() == 0 )
        {
            osg::Group* group = getMapNode()->getOverlayDecorator()->getGroup<ClampingTechnique>();
            if ( group )
                group->addChild( _overlayProxyContainer.get() );
        }
        else if ( !_clamped && _overlayProxyContainer->getNumParents() > 0 )
        {
            osg::Group* group = getMapNode()->getOverlayDecorator()->getGroup<ClampingTechnique>();
            if ( group )
                group->removeChild( _overlayProxyContainer.get() );
...
void
ClampableNode::setClamped( bool clamped )
{    
    if ( clamped != _clamped && getMapNode() )
    {        
        _newClamped = clamped;
        if ( !_dirty )
        {
...
bool
ClampableNode::addChild( osg::Node* child )
{
...
bool
ClampableNode::insertChild( unsigned i, osg::Node* child )
{
...
bool
ClampableNode::removeChild( osg::Node* child )
{
...
bool
ClampableNode::replaceChild( osg::Node* oldChild, osg::Node* newChild )
{

...
void
FeatureSourceClient::setFeatureSource(FeatureSource* fs)
{
    _featureSource = fs;
}

void
FeatureSourceClient::addedToMap(const optional<std::string>& layerName, const Map* map)
{
    if (layerName.isSet())
    {
        ...
        _featureLayerListener.listen(
            ...
            layerName.get(),
            this,
            &FeatureSourceClient::setFeatureSource);
    }
}

void
FeatureSourceClient::removedFromMap(const Map*)
{
    _featureLayerListener.clear();
}

Status
FeatureSourceClient::open(const optional<FeatureSource::Options>& options, const osgDB::Options* readOptions)
{
    if (options.isSet())
    {
        FeatureSource* fs = FeatureSource::create(options.get());
        if (fs)
        {
            fs->setReadOptions(readOptions);
...
void
FeatureSourceClient::fromConfig(const Config& conf, optional<std::string>& layerName, optional<FeatureSource::Options>& layerOptions)
{
    conf.get("features", layerName);
    conf.get("feature_source", layerName);
        ...
        ++i)
    {
        ...
        if (fs.valid())
        {
            layerOptions = FeatureSource::Options(*i);
            break;
        }
    }
}
...
void
FeatureSourceClient::getConfig(Config& conf, const optional<std::string>& layerName, const optional<FeatureSource::Options>& layerOptions)
{
    conf.set("features", layerName);
    if (layerOptions.isSet())
        conf.set(layerOptions->getConfig());
}

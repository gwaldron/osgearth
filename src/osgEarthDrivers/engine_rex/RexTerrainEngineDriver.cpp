...
namespace osgEarth { namespace Drivers { namespace RexTerrainEngine
{
    ...
    public:
        ...
        {
            _profiling = 0;
            const char* p = ::getenv("OSGEARTH_ENGINE_PROFILE");
            if ( p )
        ...
        {
            return
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_rex" ) ||
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_rex_tile" ) ||
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_rex_standalone_tile" );
        }
        ...
        virtual ReadResult readObject(const std::string& uri, const Options* options) const
        {
            if ( "osgearth_engine_rex" == osgDB::getFileExtension( uri ) )
            {
                ...
                {
                    RexTerrainEngineOptions terrainOpts;
        ...
        virtual ReadResult readNode(const std::string& uri, const Options* options) const
        {
            ...
            if ( acceptsExtension(ext) )
            {
                ...
                RexTerrainEngineNode::getEngineByUID( (UID)engineID, engineNode );
                if ( engineNode.valid() )
                {
                    ...
                    if ( "osgearth_engine_rex_tile" == ext )
                    {
                        node = engineNode->createNode(key, progress);
                    }
                    else if ( "osgearth_engine_rex_standalone_tile" == ext )
                    {

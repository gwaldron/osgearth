...
LayerDrawable::drawImplementationIndirect(osg::RenderInfo& ri) const
{
    ...
    if (_tiles != cs._previous_tiles)
    {
        ...
        cs.commands.clear();
        cs.commands.reserve(_tiles.size());
        ...
        for (auto& tile : _tiles)
        {
            ...
            {
                osg::Vec3f v = (*verts)[i] * (*tile._modelViewMatrix);
                buf.verts[4 * i + 0] = v.x();
                buf.verts[4 * i + 1] = v.y();
                buf.verts[4 * i + 2] = v.z();
        ...
        cmd.instanceCount = _tiles.size(); // one tile per command
        ...
        GLsizei tileBufSize = sizeof(TileBuffer) * cs.tilebuf.size();
